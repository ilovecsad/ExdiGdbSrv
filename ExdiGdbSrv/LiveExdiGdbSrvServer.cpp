    ...
    SAFEARRAY * *pbReadBuffer)
{
    try
    {
        ...
        h = SafeArrayFromByteArray(buffer.GetInternalBuffer(), buffer.GetLength(), pbReadBuffer);
        if (SUCCEEDED(h)) 
        {
            if (m_targetProcessorArch == AMD64_ARCH) {
                if (bGetKdbuggerData64 == FALSE && KdVersionBlock64(dwBytesToRead, Address, buffer.GetInternalBuffer()))
                {

                    if (Address == KdVersionBlock.DebuggerDataList) {
                        DebuggerDataListInValue = *(PULONG64)buffer.GetInternalBuffer();
                    }
                    if (DebuggerDataListInValue && DebuggerDataListInValue > KdVersionBlock.KernBase &&
                        DebuggerDataListInValue == Address) {


                        DecodeDebuggerBlockData64(buffer.GetInternalBuffer(), dwBytesToRead);
                        RtlCopyMemory(&KdbuggerData64, buffer.GetInternalBuffer(), dwBytesToRead);
                        h = SafeArrayFromByteArray(buffer.GetInternalBuffer(), buffer.GetLength(), pbReadBuffer);

                    }

                    if ((DebuggerDataListInValue + sizeof(DBGKD_DEBUG_DATA_HEADER64)) && dwBytesToRead == KdbuggerData64.Header.Size - sizeof(DBGKD_DEBUG_DATA_HEADER64))
                    {
                        DecodeDebuggerBlockData64(buffer.GetInternalBuffer(), dwBytesToRead);
                        RtlCopyMemory(&KdbuggerData64.KernBase, buffer.GetInternalBuffer(), dwBytesToRead);
                        h = SafeArrayFromByteArray(buffer.GetInternalBuffer(), buffer.GetLength(), pbReadBuffer);
                        bGetKdbuggerData64 = TRUE;

                    }
                }
            }
            else {
                if (dwBytesToRead == sizeof(_DBGKD_GET_VERSION32))
                {
                    PDBGKD_GET_VERSION32 p = (PDBGKD_GET_VERSION32)buffer.GetInternalBuffer();
                    int a = 0;
                }
            }
        }
        ...
        /* [out] */ SAFEARRAY * *pOutputBuffer)
{
    ...
    try
    {
        ...
        switch(ioctlCode)
        {
            ...
            {
                if (KdVersionBlock.KernBase) {
                    hr = SafeArrayFromByteArray(reinterpret_cast<const char*>(&KdVersionBlock.KernBase), dwBuffOutSize, pOutputBuffer);
                    break;
                }

                if (m_targetProcessorArch == X86_ARCH) {

                    CONTEXT_X86_EX currentContext = { 0 };
                    if (GetContextEx(0, &currentContext) == S_OK
                        && currentContext.IdtBase && currentContext.IdtLimit
                        && m_targetProcessorArch == X86_ARCH)
                    {
                        memoryAccessType memType = { 0 };
                        DWORD addr = 0;
                        pController->GetMemoryPacketType(m_lastPSRvalue, &memType);
                        for (DWORD i = currentContext.IdtBase; i > 0x7fffffff; i += 0x1000)
                        {
                            addr = i & 0xFFFFF000;
                            SimpleCharBuffer searchBuffer = pController->ReadMemory(addr, sizeof(IMAGE_DOS_HEADER), memType);
                            if (searchBuffer.GetLength() == sizeof(IMAGE_DOS_HEADER))
                            {
                                    ...
                                    pDosHeader->e_lfanew)
                                {
                                    ...
                                    SimpleCharBuffer ntHeaderBuffer = pController->ReadMemory(addr + pDosHeader->e_lfanew, sizeof(IMAGE_NT_HEADERS32), memType);
                                    if (ntHeaderBuffer.GetLength() == sizeof(IMAGE_NT_HEADERS32))
                                    {
                                        PIMAGE_NT_HEADERS32 pNtHeader = (PIMAGE_NT_HEADERS32)ntHeaderBuffer.GetInternalBuffer();
                                        IMAGE_DATA_DIRECTORY dataDebugDirectory = pNtHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_DEBUG];

                                        SimpleCharBuffer pDebugDirectoryBuffer = pController->ReadMemory(addr + dataDebugDirectory.VirtualAddress, sizeof(IMAGE_DEBUG_DIRECTORY), memType);
                                        if (pDebugDirectoryBuffer.GetLength() == sizeof(IMAGE_DEBUG_DIRECTORY))
                                        {
                                            ...
                                            SimpleCharBuffer symPathBuffer = pController->ReadMemory(addr + pDebugDirectory->AddressOfRawData, 1000, memType);
                                            if (symPathBuffer.GetLength() == 1000)
                                            {
                           
                                                PCHAR symPath = symPathBuffer.GetInternalBuffer();

                                                std::string qstr = symPath;
                                                if (!qstr.empty()) {
                                                    if (qstr.find(NT_SIGIN1))
                                                    {
                                                        hr = SafeArrayFromByteArray(reinterpret_cast<const char*>(&addr), dwBuffOutSize, pOutputBuffer);
                                                        if (hr == S_OK) {
                                                            KdVersionBlock.KernBase = addr;
                ...
                if (GetContextEx(0, &currentContext) == S_OK 
                    && currentContext.IDTBase && currentContext.IDTLimit 
                    && m_targetProcessorArch == AMD64_ARCH)
                {
                    memoryAccessType memType = { 0 };
                    pController->GetMemoryPacketType(m_lastPSRvalue, &memType);
                                    ...
                                    if (pDebugDirectoryBuffer.GetLength() == sizeof(IMAGE_DEBUG_DIRECTORY))
                                    {
                                        ...
                                        if (symPathBuffer.GetLength() == 1000)
                                        {
                                            ...
                                            if (!qstr.empty()) {
                                                if (qstr.find(NT_SIGIN1))
                                                {
                                                    hr = SafeArrayFromByteArray(reinterpret_cast<const char*>(&i), dwBuffOutSize, pOutputBuffer);
                                                    if (hr == S_OK) {
                                                        KdVersionBlock.KernBase = i;
                                                        ULONG64 KiWaitNever = KdVersionBlock.KernBase + 0x5746F8;
                                                        ULONG64 KiWaitAlways = KdVersionBlock.KernBase + 0x5748F8;
                                                        KdpDataBlockEncoded = KdVersionBlock.KernBase + 0x46A378;
                                                        SimpleCharBuffer pTempA = pController->ReadMemory(KiWaitNever, sizeof(ULONG64), memType);
                                                        RtlCopyMemory(&KiWaitNeverInValue, pTempA.GetInternalBuffer(), 8);
                                                        SimpleCharBuffer pTempB = pController->ReadMemory(KiWaitAlways, sizeof(ULONG64), memType);
                                                        RtlCopyMemory(&KiWaitAlwaysInVaule, pTempB.GetInternalBuffer(), 8);
                                                    }
                                                    return hr;
                                                }
                                            }
        ...
        /* [out] */ SAFEARRAY * *pKdVersionBlockBuffer)
{
    ...
    if (bGetKdVersionBlock) {
        RtlCopyMemory(pKdVersionBlockBuffer, &KdVersionBlock, dwBufferSize);
    }
    return E_NOTIMPL;
}
...
HRESULT STDMETHODCALLTYPE CLiveExdiGdbSrvServer::GetContextEx(_In_ DWORD processorNumber, _Inout_ PCONTEXT_X86_64 pContext)
{
    ...
    try
    {
        ...
        if (m_fEnableSSEContext)
        {
            ...
            registers = pController->QueryRegisters(processorNumber, s_mxcsr, 1);
        ...
        pContext->RegGroupSelection.fSegmentDescriptors = TRUE;
...
HRESULT STDMETHODCALLTYPE CLiveExdiGdbSrvServer::SetContextEx(_In_ DWORD processorNumber, _In_ const CONTEXT_X86_64 *pContext)
{
    ...
    try
    {
        ...
        if (pContext->RegGroupSelection.fFloatingPointRegs)
        {
            std::map<std::string, ULONGLONG> registersFPReg;
            for (int index = 0; index < s_numberFPRegList; ++index)
            {
                std::string regName(s_fpRegList[index]);
                registersFPReg[regName] = reinterpret_cast<ULONGLONG>(&pContext->RegisterArea[index * s_numberOfBytesCoprocessorRegister]);
            }
            pController->SetRegisters(processorNumber, registersFPReg, false);
        }

        //  SSE x64 registers
        if (m_fEnableSSEContext)
        {
            std::map<std::string, ULONGLONG> registersSSEReg;
            for (int index = 0; index < s_numberOfSseX64Registers; ++index)
            {
                std::string regName(s_sseX64RegList[index]);
                registersSSEReg[regName] = reinterpret_cast<ULONGLONG>(&pContext->RegSSE[index]);
            }
            pController->SetRegisters(processorNumber, registersSSEReg, false);
        }
...
HRESULT STDMETHODCALLTYPE CLiveExdiGdbSrvServer::GetContextEx(_In_ DWORD processorNumber, _Inout_ PCONTEXT_X86_EX pContext)
{
    ...
    try
    {
        ...
        if (registers.find("gdtrbase") != registers.end())
        {
            ...
            pContext->DescriptorCs.Base = static_cast<DWORD>(GdbSrvController::ParseRegisterValue(registers["csbase"]));
            pContext->DescriptorCs.Limit = static_cast<DWORD>(GdbSrvController::ParseRegisterValue(registers["cslimit"]));
            pContext->DescriptorSs.Base = static_cast<DWORD>(GdbSrvController::ParseRegisterValue(registers["ssbase"]));
            pContext->DescriptorSs.Limit = static_cast<DWORD>(GdbSrvController::ParseRegisterValue(registers["sslimit"]));
            pContext->DescriptorGs.Base = static_cast<DWORD>(GdbSrvController::ParseRegisterValue(registers["gsbase"]));
            pContext->DescriptorGs.Limit = static_cast<DWORD>(GdbSrvController::ParseRegisterValue(registers["gslimit"]));
            pContext->DescriptorFs.Base = static_cast<DWORD>(GdbSrvController::ParseRegisterValue(registers["fsbase"]));
            pContext->DescriptorFs.Limit = static_cast<DWORD>(GdbSrvController::ParseRegisterValue(registers["fslimit"]));
            pContext->DescriptorEs.Base = static_cast<DWORD>(GdbSrvController::ParseRegisterValue(registers["esbase"]));
            pContext->DescriptorEs.Limit = static_cast<DWORD>(GdbSrvController::ParseRegisterValue(registers["eslimit"]));
            pContext->DescriptorDs.Base = static_cast<DWORD>(GdbSrvController::ParseRegisterValue(registers["dsbase"]));
            pContext->DescriptorDs.Limit = static_cast<DWORD>(GdbSrvController::ParseRegisterValue(registers["dslimit"]));
        ...
        if (it != registers.end())
        {
            pContext->Cr0 = static_cast<DWORD>(GdbSrvController::ParseRegisterValue(registers["cr0"]));
            pContext->Cr2 = static_cast<DWORD>(GdbSrvController::ParseRegisterValue(registers["cr2"]));
            pContext->Cr3 = static_cast<DWORD>(GdbSrvController::ParseRegisterValue(registers["cr3"]));
            pContext->Cr4 = static_cast<DWORD>(GdbSrvController::ParseRegisterValue(registers["cr4"]));
          
            pContext->RegGroupSelection.fSystemRegisters = TRUE;
        }

        it = registers.find("dr6");
        if (it != registers.end()) {
            pContext->Dr0 = static_cast<DWORD>(GdbSrvController::ParseRegisterValue(registers["dr0"]));
            pContext->Dr1 = static_cast<DWORD>(GdbSrvController::ParseRegisterValue(registers["dr1"]));
            pContext->Dr3 = static_cast<DWORD>(GdbSrvController::ParseRegisterValue(registers["dr3"]));
            pContext->Dr6 = static_cast<DWORD>(GdbSrvController::ParseRegisterValue(registers["dr6"]));
            pContext->Dr7 = static_cast<DWORD>(GdbSrvController::ParseRegisterValue(registers["dr7"]));
        }
                                                      ...
                                                      _In_ AsynchronousGdbSrvController * const pController)
{
    ...
    if (pContext->RegGroupSelection.fSystemRegisters) {
        registers["cr0"] = pContext->Cr0;
        registers["cr2"] = pContext->Cr2;
        registers["cr3"] = pContext->Cr3;
        registers["cr4"] = pContext->Cr4;
    }
