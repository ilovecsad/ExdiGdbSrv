...
static void QRcmdRegistor3(GdbSrvController* ctx, const std::string& input, std::map<std::string, std::string>& maps)
{
    std::string qstr = std::string("qRcmd,").append(string_to_hex(input));
    std::string rstr = ctx->ExecuteCommand(qstr.c_str());
    if (!rstr.empty()
        ...
        && rstr.c_str()[0] == 'O')
    {
        ...
        size_t tag1index = sstr.find("base 0x");
        size_t tag2index = sstr.find("limit 0x");
        size_t tag3index = sstr.find("type");
        if (tag1index > 0 && tag1index != (size_t)(-1) &&
            tag2index > 0 && tag2index != (size_t)(-1))
        {
            std::string value1 = sstr.substr(tag1index + 7, sstr.size() - tag1index - 7 - (sstr.size() - tag2index));
            std::string value2 = sstr.substr(tag2index + 9, sstr.size() - tag1index - 7 - (sstr.size() - tag2index) - 2);

            std::string key1 = std::string(sstr.substr(0, 2)).append("base");
            std::string key2 = std::string(sstr.substr(0, 2)).append("limit");
...
static void QuerySpecialRegistor(GdbSrvController* ctx, std::map<std::string, std::string>& maps)
{
    ...
    QRcmdRegistor(ctx, "r dr0", maps);
    QRcmdRegistor(ctx, "r dr1", maps);
    QRcmdRegistor(ctx, "r dr2", maps);
    QRcmdRegistor(ctx, "r dr3", maps);
    QRcmdRegistor(ctx, "r dr6", maps);
    QRcmdRegistor(ctx, "r dr7", maps);

    QRcmdRegistor2(ctx, "r idtr", maps);
    QRcmdRegistor2(ctx, "r gdtr", maps);

    QRcmdRegistor3(ctx, "r cs", maps);
    QRcmdRegistor3(ctx, "r ss", maps);
    QRcmdRegistor3(ctx, "r gs", maps);
    QRcmdRegistor3(ctx, "r fs", maps);
    QRcmdRegistor3(ctx, "r es", maps);
    QRcmdRegistor3(ctx, "r ds", maps);
